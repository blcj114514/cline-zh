...
| `dict.json` | **合并后词库（1324 条）**，按 key 长度降序，先匹配长句 |
| `rules.json` | **50 条正则规则**，处理带变量的动态文案（厂商名、`(China)`、`Latest` 后缀、`Provider：X` 等） |
...
16. **补齐 45 条真实界面文案**（`dict-batch6.json`）+ **3 条新规则**（`Provider：X` / `Environment：X` / `Sort sessions：X`）：这些字符串当初未被抽取进语料，是真机实测时从**活的 Cline 界面**里逐条捞出来的（含侧边栏、按钮、`aria-label`、输入框 `placeholder`）。
